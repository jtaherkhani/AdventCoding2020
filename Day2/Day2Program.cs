...
namespace Day2
{
    class Day2Program
    {
        ...
        private const int ExpectedCharacter = 1;
        ...
        static void Main(string[] args)
        {
            ...
            foreach (string passwordAndMetadata in Values)
            {
                CleanPasswordAndMetadata(passwordAndMetadata);
                var expectedValueSets = passwordAndMetadata.Split(' ', StringSplitOptions.RemoveEmptyEntries);

                (int lowerBounds, int upperBounds) = ParseBounds(expectedValueSets[Bounds]); // if I were to check this in somewhere these variable names should be more generic
                char characterToCheck = expectedValueSets[ExpectedCharacter].ToCharArray().First();
                char[] passwordChars = expectedValueSets[Password].ToCharArray();

                if (IsPasswordValidForRangePolicy(lowerBounds, upperBounds, characterToCheck, passwordChars))
                {
                    validPasswordsPart1++;
                }

                if (IsPasswordValidForCharacterTestPolicy(lowerBounds, upperBounds, characterToCheck, passwordChars))
                {
                    validPasswordsPart2++;
                }
            }

            Console.WriteLine(string.Format("There are {0} valid passwords for part 1", validPasswordsPart1));
            Console.WriteLine(string.Format("There are {0} valid passwords for part 1", validPasswordsPart2));
        }

        static string CleanPasswordAndMetadata(string passwordAndMetadata) =>
            passwordAndMetadata.Replace(":", "").Trim();

        static (int, int) ParseBounds(string bounds)
        {
            var boundValues = bounds.Split('-', StringSplitOptions.RemoveEmptyEntries);

            if (boundValues.Length != 2)
            {
                throw new Exception("Unacceptable numbers of bounds");
            }

            return (int.Parse(boundValues[0]), int.Parse(boundValues[1]));
        }

        static bool IsPasswordValidForRangePolicy(int lowerBounds, int upperBounds, char characterToCheck, char[] password)
        {
            int occurances = password.Count(x => x == characterToCheck);

            return (occurances >= lowerBounds) && (occurances <= upperBounds);
        }

        static bool IsPasswordValidForCharacterTestPolicy(int firstPositionToMatch, int secondPositionToMatch, char characterToCheck, char[] password)
        {
            if (password.Length < firstPositionToMatch || password.Length < secondPositionToMatch)
            {
                return false;
            }

            // values have to be zero adjusted
            firstPositionToMatch--;
            secondPositionToMatch--;

            bool firstCharacterMatches = password[firstPositionToMatch] == characterToCheck;
            bool secondCharacterMatches = password[secondPositionToMatch] == characterToCheck;

            return firstCharacterMatches != secondCharacterMatches; // could be single lined but I find this more readable
        }
