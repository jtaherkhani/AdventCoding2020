...
namespace Day4
{
    public class Day4Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (HasValidFields(passport) && HasValidFieldValues(passport))
                {
                    validPassportsCount++;
                }
            }

            Console.WriteLine(string.Format("There are {0} valid passports", validPassportsCount));
        }

        private static List<string> GetPassports() =>
            Input.Trim().Split(new[] { Environment.NewLine + Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries).ToList();

        private static bool HasValidFields(string passport)
        {
            foreach (var idToCheck in IdsToCheck)
            {
                if (!passport.Contains(idToCheck))
                {
                    return false;
                }
            }
        ...
        private static bool HasValidFieldValues(string passport)
        {
            var passportFieldsDictionary = CreatePassportFieldsDictionary(passport);

            foreach (var idToCheck in IdsToCheck)
            {
                if (!HasValidPassportValues(idToCheck, passportFieldsDictionary[idToCheck]))
                {
                    return false;
                }
            }
        ...
        private static Dictionary<string, string> CreatePassportFieldsDictionary(string passport) =>
            passport.Split(new[] { " ", Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries)
                .Where(splitValues => !splitValues.Contains(CountryId))
                .Select(kvArray => kvArray.Split(':', 2, StringSplitOptions.RemoveEmptyEntries))
                .ToDictionary(kvArray => kvArray[0], kvArray => kvArray[1], StringComparer.InvariantCultureIgnoreCase);


        private static bool HasValidPassportValues(string key, string valueToValidate) =>
            key switch
            {
                BirthYear => IsValueInRange(1920, 2002, valueToValidate),
                IssueYear => IsValueInRange(2010, 2020, valueToValidate),
                ExpirationYear => IsValueInRange(2020, 2030, valueToValidate),
                Height => IsHeightValid(valueToValidate),
                HairColor => valueToValidate.Length == 7 && valueToValidate.Substring(0, 1) == "#" && valueToValidate[1..].All(char.IsLetterOrDigit),
                EyeColor => EyeColors.Contains(valueToValidate),
                PassportId => valueToValidate.Length == 9 && valueToValidate.All(char.IsNumber),
        ...
        private static bool IsHeightValid(string height)
        {
            ...
            {
                return IsValueInRange(59, 76, height[0..^2]);
            }
            else if (height.Contains("cm"))
            {
                return IsValueInRange(150, 193, height[0..^2]);
            }
        ...
        private static bool IsValueInRange(int lower, int higher, string valueToValidate)
        {
            int value = Convert.ToInt16(valueToValidate);
            return lower <= value
                && higher >= value;
        }

        private static readonly List<string> IdsToCheck = new List<string>
        {
            BirthYear,
            IssueYear,
            ExpirationYear,
